...
// Objective 11: Maze solving using BFS, Graph mapped using Adjacency List.
// Array A of size n x m treated as a maze.
// Path from start (0,0) to end (n-1,m-1).
...
int main() {
    ...
    // Dynamically allocate 2D array grid as required
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            ...
            // High chance of 0 to allow path finding
            if (randNum < 80) {
                grid[i][j] = 0;
            } else {
                grid[i][j] = 1;
    ...
    // Force start and end points to be open 0
    grid[0][0] = 0;
    grid[n-1][m-1] = 0;
