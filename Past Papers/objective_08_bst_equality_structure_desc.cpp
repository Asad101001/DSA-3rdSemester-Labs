...
// Objective 07: Binary Search Trees
// 1. Construct two binary search trees.
// 2. Check if they are Equal.
// 3. Check if they have the same structure.
// 4. Sort the nodes in descending order.
...
// Check if Equal (Same Structure AND Same Data)
...
// Check if Same Structure (Ignoring Data)
...
// Print nodes in descending order (Reverse In-Order Traversal)
...
int main() {
    ...
    // Construct Tree B (Identical to A)
    insert(&treeB, 50);
    insert(&treeB, 30);
    insert(&treeB, 70);
    
    // Construct Tree C (Different data, same structure)
    insert(&treeC, 60);
    insert(&treeC, 40);
    insert(&treeC, 80);
